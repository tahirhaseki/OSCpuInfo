...
void param2(){
  ...
  printk("\tSystem was booted since : %d Hours %d Minutes %d Seconds\n",hours,mins,seconds);
  hours = idle / 360;
  mins = ((int)idle % 360) / 60;
  seconds = idle - (hours * 360 + mins * 60);
  printk("\tSystem has been idle since : %d Hours %d Minutes %d Seconds\n",hours,mins,seconds);
  printk("\tThe total number of active tasks : %s\n",active);
